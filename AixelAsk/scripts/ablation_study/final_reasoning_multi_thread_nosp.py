...
from scripts.get_sub_table import retrieve_final_subtable, retrieve_final_subtable_add_noplan
from concurrent.futures import ThreadPoolExecutor, as_completed
from scripts.generate_answer import generate_final_answer, generate_noplan_answer
...
def process_single_table(index, d, row_prompt, col_prompt, plan_prompt, final_reasoning_prompt, noplan_reasoning_prompt):
    ...
    # # Example: clean header (kept for reference; currently unused)
    # header = table[0]
    ...
    try:
        # Generate natural-language descriptions for rows/columns
        ...
        # # Generate a solution plan (disabled in this script)
        # solution_plan = get_solution_plan(cleaned_table, question, plan_prompt)
        # solution_plan = ""

        # If there are multiple stages and the plan is valid, perform retrieval
        final_subtable, final_row_indices, final_col_indices = retrieve_final_subtable_add_noplan(
            indexed_cleaned_table, row_descriptions, col_descriptions, request_gpt_embedding, question
        )
        final_answer = generate_noplan_answer(question, final_subtable, noplan_reasoning_prompt)
        ...
        print(f"Error encountered at index {index}: {e}.")
...
def main():
    ...
    # Read existing result file to collect indices already processed
    result_file_path = "result/ablation_study/wtq_plus/noplan/result_1120_valid_test_gpt3_5.jsonl"
